=== Log Book WordPress Free Theme ===
Contributors: thememiles
Requires at least: 4.6
Tested up to: 5.1.1
Stable tag: 1.0.0
Requires PHP: 5.6
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html
Tags: one-column, two-columns, right-sidebar, custom-colors, rtl-language-support, sticky-post, threaded-comments, translation-ready, blog, news, entertainment

Here is a short description of the theme.

== Description ==

Log Book is a clean, minimal and SEO friendly and WooCommerce compatible  theme which is perfect for writers who need to create personal blog site with simple creative features and effects to make readers feel the pleasure of reading blog posts and articles, Log Book theme Comes with classic styles and will help you create a simple and clean blog, if you are a blogger, then it’s a perfect choice for you if you don’t need to have any experiment to setup your WordPress personal blog, it’s super simple and easy to setup, you will get high quality, responsive, well crafted blog out of the box to make writers only focuses on writing content, and it has great typography to make your fans and followers focus on every word you write. It’s Best Free Themes and Seo Friendly Themes,Work with the most popular page builders as Elementor, Beaver Builder, Brizy, Visual Composer, Divi, SiteOrigin, etc.


== Installation ==

This section describes how to install the theme and get it working.

e.g.

1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload and Choose File, then select the theme's .zip file. Click Install Now
3. Click Activate to use your new theme right away


== Frequently Asked Questions ==

= Does this theme support any plugins? =

   Supports Contact Form 7.
   Supports Jetpack.
   Supports Mailchimp.
   Support Woocommerce.


==  Theme License & Copyright ==

Log Book is based on Underscores http://underscores.me/, (C) 2012-2014 Automattic, Inc.
Unless otherwise specified, all the theme files, scripts and images are licensed under GNU General Public License Version 2 or later.
License: GNU General Public License v2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Log Book WordPress Theme, Copyright 2019 ThemeMiles
It is distributed under the terms of the GNU GPL

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful.

Log Book bundles the following third-party resources:

== Credits ==

Based on Underscores,(C) 2012-2016 Automattic, Inc.
License: GPLv2
Source: https://www.gnu.org/licenses/gpl-2.0.html

dlmenu Licensed under the MIT license.
Licenses: MIT/GPL2
Source: http://www.opensource.org/licenses/mit-license.php

Modernizr Licensed under the MIT license.
Licenses: MIT/GPL2
Source: https://modernizr.com/license/

Font Awesome icons, Copyright Dave Gandy
Icons License: CC BY 4.0 
Fonts License: SIL OFL 1.1 
Source: http://fontawesome.io/

Bootstrap, Copyright 2011-2018 Twitter
License: MIT
Source: https://getbootstrap.com/

Licensed under the MIT license.
License: MIT
https://github.com/evenicoulddoit/owl-carousel/blob/master/LICENSE

BreadcrumbTrail Copyright (c) 2008 - 2015, Justin Tadlock
License: Gpl
http://www.gnu.org/licenses/old-licenses/gpl-2.0.html

TGM-Plugin-Activation Copyright (c) 2011, Thomas Griffin
License: Gpl
http://tgmpluginactivation.com/


== Changelog ==

= 1.0.8 - july 29 2020 =

* Fixed Design issues
* Recommend Gutenberg Plugin

= 1.0.7 - April 18 2020 =

* Fixed Breadcrumb error in Google Console issues.
* Add Page Builder Template
* Fixed detail page desing issues.
* Fixed Sidebar design issues

= 1.0.6 - March 26 2020 =

* Plugin require change to recommend.
* Keyboard navigation issues fixed.

= 1.0.5 - March 2 2020 =

* Fix Image issues on sigle page
* Add tgm Plugin for demo import

= 1.0.4 - January 9 2020 =

* Add Option Hide or Show Sticky Sidebar
* Fix Image issues on sigle page
* Fix Font issues on Responsive


= 1.0.3 - January 6 2020 =

* Add Option to Hide Red Header Part on each page
* Fix minor Design issues

= 1.0.2 - December 5 2019 =

* Changed Screenshot 
* Add screenshot image url
* Made WooCommerce compatible  

= 1.0.1 - November 25 2019 =

* Fixed sanitization issues
* Changed Screenshot 
* Add screenshot image url
* Add both minify and unminify css and js
* Fixed Layout issues.

= 1.0.0 - August 15 2019 =
* Submitted to WordPress

== Upgrade Notice == 

* There is not any upgrade notic now. 

== Screenshots ==

* https://www.pexels.com/photo/scenic-view-of-city-during-dawn-2138001

* https://www.pexels.com/photo/cooked-foods-on-wooden-plate-2170313

* https://www.pexels.com/photo/group-of-people-having-neon-party-1684187




